...
int type_check(ast * t, PclType ftype) {
	...
	if (!(t->left)) {
	  ...
	  if (t->right->type->kind != TYPE_POINTER  || t->right->type->refType->kind != TYPE_IARRAY || !t->right->type->refType->refType->full) {
	    printf("Type Error: \"new [size]\" argument must be pointer to indefinite array of a full type!\n");
	    return 1;
	  }
	}
	break;
	...
	case STMT:
	if (!(p = lookupEntry(t->id, LOOKUP_CURRENT_SCOPE, true)) || p->u.eVariable.type->kind != TYPE_LABEL){
	...
	case CALL:
	if (t->left && type_check(t->left, ftype)) return 1;
	if (!(p = lookupEntry(t->id, LOOKUP_ALL_SCOPES, true)) || p->entryType != ENTRY_FUNCTION){
	...
	case NIL:{
		printf("Error (nil): \"nil\" should only appear as rvalue of an assignment!\n");
		return 1;
	}
